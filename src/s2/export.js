...
const PARTICIPANT_COLUMNS = [
  ...
  "complete_pass", "attention_pass", "text_chars", "answered_count", "started_at", "first_answer_at", "last_answer_at",
...
export default async function s2ExportRoutes(app) {
  ...
  app.get("/api/s2/export/wide.csv", async (req, reply) => {
    ...
    const meta = ["participant_id", "short_code", "seg_order", "pos_REL", "pos_ADV", "pos_COL",
                  "status", "source", "external_pid", "is_test", "complete_pass", "attention_pass", "text_chars",
    ...
    for await (const batch of scanParticipants(where, params)) {
      ...
      for (const p of batch) {
        ...
        const metaVals = [
          ...
          p.status, p.source, p.external_pid, p.is_test, p.complete_pass, p.attention_pass, p.text_chars,
